...
const ComponentStatusesCtx = createContext<ComponentStatuses | undefined>(undefined);
...
// Component-to-mesh mapping for blueprint highlighting
type MeshComponent = "build_bed" | "control_tower" | "top_hood" | "powder_port" | "front_cabinets" | "side_rear";

const COMPONENT_TO_MESH: Record<DotKey, MeshComponent> = {
  blade: "build_bed",
  motor: "control_tower",
  rail: "build_bed",
  nozzle: "powder_port",
  ...
  heater: "side_rear",
  sensor: "side_rear",
  insulation: "side_rear",
};

// Transform hotspot from model-local → world space
// Outer group: position=[0,0,0], rotation=[0,-0.42,0]
...
function AlertDot({
  ...
  return (
    <group position={position}>
      ...
      {/* Only show sphere indicators in non-blueprint mode */}
      {visible && !bp && (
        <>
...
function Material({
  ...
  const bp = useContext(BlueprintCtx);
  const statuses = useContext(ComponentStatusesCtx);

  // Find the worst status for this mesh component (only in blueprint mode)
  let worstStatus: CompStatus | null = null;
  let highlightColor: string | null = null;

  if (bp && meshComponent && statuses) {
    // Find all components that map to this mesh
    const relatedComponents = (Object.keys(COMPONENT_TO_MESH) as DotKey[]).filter(
      key => COMPONENT_TO_MESH[key] === meshComponent
    );

    // Find the worst status among them
    for (const compKey of relatedComponents) {
      const status = statuses[compKey];
      if (statusNeedsRedHighlight(status)) {
        if (!worstStatus ||
            (status === "CRITICAL" || status === "FAILED") ||
            (worstStatus !== "CRITICAL" && worstStatus !== "FAILED" && status === "DEGRADED") ||
            (worstStatus === "WARNING" && status !== "WARNING")) {
          worstStatus = status;
          highlightColor = statusDotColor(status);
        }
      }
    }
  }
  ...
  if (bp) {
    // Blueprint mode with status highlighting
    if (highlightColor) {
      return (
        <>
          <meshStandardMaterial
  ...
  // Normal mode - no highlighting, always use default appearance
  return (
  ...
  return (
    <group position={[-1.82, 1.09, 0]}>
      <RoundedBox args={[0.72, 2.18, 0.72]} radius={0.035} smoothness={5}>
        <Material color="#151718" metalness={0.46} roughness={0.28} clearcoat={0.26} meshComponent="control_tower" />
      </RoundedBox>
      <RoundedBox args={[0.52, 0.34, 0.045]} radius={0.018} position={[0, 0.54, 0.385]} smoothness={4}>
        <Material color="#0b0d0e" metalness={0.28} roughness={0.24} meshComponent="control_tower" />
      </RoundedBox>
      {!bp && (
      ...
      <RoundedBox args={[0.25, 0.13, 0.04]} radius={0.014} position={[0, 0.18, 0.385]} smoothness={4}>
        <Material color="#0c1215" metalness={0.32} roughness={0.3} emissive="#22b9ff" emissiveIntensity={0.09} meshComponent="control_tower" />
      </RoundedBox>
      {[-0.42, -0.35, -0.28, -0.21, -0.14, -0.07].map((y) => (
        <mesh key={y} position={[0.05, y, 0.388]}>
          <boxGeometry args={[0.31, 0.018, 0.02]} />
          <Material color="#070809" metalness={0.18} roughness={0.54} meshComponent="control_tower" />
        </mesh>
      ))}
      <RoundedBox args={[0.2, 0.58, 0.16]} radius={0.02} position={[-0.38, -0.62, -0.25]} smoothness={4}>
        <Material color="#101314" metalness={0.25} roughness={0.42} meshComponent="control_tower" />
      </RoundedBox>
    </group>
  );
}
  ...
  return (
    <group>
      <RoundedBox args={[2.92, 0.3, 0.98]} radius={0.028} position={[0.37, 1.55, 0]} smoothness={4}>
        <Material color="#393b3b" metalness={0.42} roughness={0.25} clearcoat={0.18} meshComponent="top_hood" />
      </RoundedBox>
      {panels.map((x) => (
        <mesh key={x} position={[x, 1.55, 0.505]}>
          <boxGeometry args={[0.018, 0.25, 0.018]} />
          <Material color="#17191a" metalness={0.34} roughness={0.34} meshComponent="top_hood" />
        </mesh>
      ))}
      <mesh position={[0.42, 1.33, 0.49]} rotation={[0.18, 0, 0]}>
        <boxGeometry args={[2.78, 0.08, 0.12]} />
        <Material color="#101314" metalness={0.38} roughness={0.24} meshComponent="top_hood" />
      </mesh>
      <RoundedBox args={[2.45, 0.1, 0.08]} radius={0.018} position={[0.47, 1.19, 0.54]} smoothness={4}>
        <Material color="#111415" metalness={0.54} roughness={0.18} clearcoat={0.18} meshComponent="top_hood" />
      </RoundedBox>
      <mesh position={[1.22, 1.2, 0.592]}>
        <boxGeometry args={[0.78, 0.035, 0.026]} />
        <Material color="#7ee3ff" metalness={0.08} roughness={0.2} emissive="#34caff" emissiveIntensity={0.36} meshComponent="top_hood" />
      </mesh>
      <mesh position={[0.17, 1.25, 0.56]}>
        <boxGeometry args={[1.08, 0.035, 0.025]} />
        <Material color="#1a1d1e" metalness={0.2} roughness={0.32} meshComponent="top_hood" />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group position={[0.05, 1.82, 0.02]}>
      <mesh>
        ...
        <Material color="#242728" metalness={0.5} roughness={0.28} meshComponent="powder_port" />
      </mesh>
      <mesh position={[0, 0.11, 0]}>
        <cylinderGeometry args={[0.19, 0.19, 0.18, 48]} />
        <Material color="#111314" metalness={0.55} roughness={0.24} meshComponent="powder_port" />
      </mesh>
      <mesh position={[0, 0.21, 0]}>
        <cylinderGeometry args={[0.22, 0.18, 0.05, 48]} />
        <Material color="#1e2021" metalness={0.58} roughness={0.26} meshComponent="powder_port" />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group>
      <RoundedBox args={[1.02, 1.05, 0.08]} radius={0.025} position={[-0.33, 0.58, 0.56]} smoothness={4}>
        <Material color="#e0e2e3" metalness={0.16} roughness={0.33} clearcoat={0.16} meshComponent="front_cabinets" />
      </RoundedBox>
      <RoundedBox args={[0.92, 0.98, 0.08]} radius={0.025} position={[0.78, 0.54, 0.56]} smoothness={4}>
        <Material color="#d7d9da" metalness={0.16} roughness={0.34} clearcoat={0.16} meshComponent="front_cabinets" />
      </RoundedBox>
      <RoundedBox args={[0.66, 1.28, 0.1]} radius={0.02} position={[-0.02, 0.45, 0.62]} smoothness={4}>
        <Material color="#e8e9e9" metalness={0.12} roughness={0.32} clearcoat={0.2} meshComponent="front_cabinets" />
      </RoundedBox>
      <HpLogoDecal />
      ...
      <mesh position={[1.31, 0.43, 0.616]}>
        ...
        <Material color="#1d2021" metalness={0.28} roughness={0.28} meshComponent="front_cabinets" />
      </mesh>
      <mesh position={[1.31, 0.43, 0.638]}>
        <boxGeometry args={[0.016, 0.46, 0.014]} />
        <Material color="#747b7e" metalness={0.24} roughness={0.24} meshComponent="front_cabinets" />
      </mesh>
      {[-0.84, -0.78, 0.74, 0.8].map((x) => (
        <mesh key={x} position={[x, 1.04, 0.616]}>
          <cylinderGeometry args={[0.015, 0.015, 0.012, 18]} />
          <Material color="#3f95d6" emissive="#3f95d6" emissiveIntensity={0.16} meshComponent="front_cabinets" />
        </mesh>
      ))}
    </group>
  );
  ...
  return (
    <group>
      <RoundedBox args={[1.35, 0.1, 0.52]} radius={0.02} position={[0.14, 1.11, 0.33]} smoothness={4}>
        <Material color="#0f1112" metalness={0.48} roughness={0.2} meshComponent="build_bed" />
      </RoundedBox>
      <mesh position={[0.14, 1.17, 0.36]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[1.12, 0.34]} />
        <Material color="#222528" metalness={0.38} roughness={0.18} meshComponent="build_bed" />
      </mesh>
      <mesh position={[0.16, 1.184, 0.522]}>
        <boxGeometry args={[0.82, 0.012, 0.012]} />
        <Material color="#caa96b" metalness={0.2} roughness={0.24} emissive="#caa96b" emissiveIntensity={0.07} meshComponent="build_bed" />
      </mesh>
      <mesh position={[0.92, 1.1, 0.35]}>
        <boxGeometry args={[0.032, 0.09, 0.52]} />
        <Material color="#303335" metalness={0.34} roughness={0.3} meshComponent="build_bed" />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group>
      <mesh position={[-2.185, 0.14, 0.05]}>
        ...
        <Material color="#d22e2e" emissive="#d22e2e" emissiveIntensity={0.18} roughness={0.25} meshComponent="side_rear" />
      </mesh>
      <mesh position={[1.67, 0.58, 0.08]}>
        <boxGeometry args={[0.035, 0.14, 0.26]} />
        <Material color="#d1d4d5" metalness={0.18} roughness={0.4} meshComponent="side_rear" />
      </mesh>
    </group>
  );
}
...
export function HpMetalJetS100Model({
  ...
  return (
    <ComponentStatusesCtx.Provider value={statuses}>
  ...
  return (
    <BlueprintCtx.Provider value={bp}>
      <Canvas camera={{ position: [4.4, 2.5, 4.9], fov: 34 }} dpr={[1, 2]} shadows={!bp}>
        ...
        <group rotation={[0, -0.42, 0]} position={[0, 0, 0]}>
          {bp ? (
            <Grid
              position={[0, -0.012, 0]}
              cellSize={0.25}
        ...
        {!bp && (
          <ContactShadows position={[0, -0.01, 0]} opacity={0.46} blur={1.8} scale={5.5} far={4} />
        )}
